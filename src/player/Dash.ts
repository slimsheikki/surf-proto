...
export class Dash {
  ...
  tick(dt: number): void {
    ...
    if (this.rechargeTimer >= DASH_RECHARGE_SECONDS) {
      this.rechargeTimer -= DASH_RECHARGE_SECONDS;
      this.charges += 1;
    }
  }
  ...
  get fraction(): number {
    const partial = this.charges < this.maxCharges ? this.rechargeTimer / DASH_RECHARGE_SECONDS : 0;
