...
- **Variants:** half = one banked face; full = A-frame (two faces meeting at a ridge);
  inverted = V channel. Full/inverted are generated by mirroring the half face — which is
  exact only for straight constant-width paths, so **curved and tapered families ship
  half-only** (a curved full needs concentric arc-length correction; a tapered full needs
  converging face paths). Deliberate, not an oversight.
...
curved/tapered families (above), spline width-awareness (a guide curve has no width, so
trapezoids are never auto-chosen), and thumbnails in `RampDefinition`.
